...
function callback(results, status) {
   ...
   if (status == google.maps.places.PlacesServiceStatus.OK) {
      ...
      for (let i = 0; i < results.length; i++) {
         document.getElementById("gyms").innerHTML += ` <div id="${i}"> <h1> ${results[i].name} <p> ${results[i].rating} </p> <div>  </div> </h1> </div>`
      }
   }
}
...
function showGyms() {
   ...
   for (let i = 0; i < myResults.length; i++) {
      gymsDiv.innerHTML += ` <div id="card${i}" class="text-light border rounded blue"> <br> <h5> ${myResults[i].name} </h5> <br> <img class="images" src="${myResults[i].photos[0].raw_reference.fife_url}"> <br> <br> <p> ${myResults[i].vicinity} </p> <div> `
      ...
      if (myResults[i].rating > 2) { document.getElementById(`card${i}`).innerHTML += `<p id="rating${i}"> Rating:  <i class="fa fa-star"></i> <i class="fa fa-star-half"></i> </p>` } else
